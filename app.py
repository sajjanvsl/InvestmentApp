...
# ------------------------------
# IMPROVED SWING SCANNER (with debug)
# ------------------------------
def improved_ai_swing_signal(df, name, debug_mode=False):
    if df.empty or len(df) < 50:
        if debug_mode:
            return None, {'RSI': 'N/A', 'ema_condition': 'N/A', 'price_condition': 'N/A', 'volume_surge': 'N/A', 'pass': False, 'error': 'Insufficient data'}
        return None
    try:
        ...
        if debug_mode:
            debug_info = {
                ...
                'price_condition': f"{current_price:.2f} > {recent_low*1.01:.2f}",
                'volume_surge': f"{volume_ratio:.1f}x",
                'pass': pass_all,
                'error': ''
            }
...
def main_app():
    ...
    # ----- Tab 1: AI Swing Scanner (with debug) -----
    with tab1:
                ...
                try:
                    ...
                    if show_debug:
                        debug_data.append({
                            ...
                            'Volume Surge >20%': f"{volume_ratio:.1f}x = {'✅' if volume_surge else '❌'}",
                    ...
                    if pass_all:
                        sig = {
                            ...
                            'Target': round(recent_low * 1.05, 2),
