...
# ------------------------------
# SCREENER FUNCTIONS WITH CRITERIA DISPLAY
# ------------------------------
def swing_pullback_signal(df, name):
    """Swing Pullback Screener with criteria tracking."""
    ...
    try:
        ...
        criteria = {
            'Close > 50 EMA': current_close > current_ema50,
            '20 EMA > 50 EMA': current_ema20 > current_ema50,
            'Close ≤ 1.02 × 20 EMA': current_close <= 1.02 * current_ema20,
            'RSI > 40': current_rsi > 40,
            'RSI < 60': current_rsi < 60,
            'Volume > 1.2× Avg': current_volume > 1.2 * current_vol_sma if current_vol_sma > 0 else False,
            'Price > 100': current_close > 100
        }
...
def swing_breakout_signal(df, name):
    """Swing Breakout Screener with criteria tracking."""
    if df.empty or len(df) < 50:
        return None, {}
    try:
        close = df['Close'].astype(float)
        ...
        criteria = {
            'Close > 20-day High': current_close > current_highest_high and prev_close <= current_highest_high,
            'Volume > 1.5× Avg': current_volume > 1.5 * current_vol_sma if current_vol_sma > 0 else False,
            'RSI > 60': current_rsi > 60,
            '50 EMA > 200 EMA': current_ema50 > current_ema200,
            'Price > 100': current_close > 100
        }
...
def intraday_breakout_signal(name):
    """Intraday Breakout Screener with criteria tracking."""
    ...
    try:
        ...
        criteria = {
            'Close > VWAP': current_close > current_vwap,
            'RSI > 55': current_rsi > 55,
            'Volume > 1.5× Avg': current_volume > 1.5 * current_vol_sma if current_vol_sma > 0 else False,
            'Close > Previous High': current_close > prev_high
        }
...
def ai_swing_signal(df, name):
    """AI Swing Scanner with criteria tracking."""
    if df.empty or len(df) < 50:
        return None, {}
    try:
        close = df['Close'].astype(float)
        ...
        criteria = {
            'RSI < 45': current_rsi < 45,
            '20 EMA > 50 EMA': ma20.iloc[-1] > ma50.iloc[-1],
            'Price > Recent Low +2%': current_price > recent_low * 1.02,
            'AI Confidence > 60%': ai_confidence > 0.6
        }
        
        all_met = rule_buy or ai_confidence > 0.6
...
def ai_intraday_signal(df, name):
    """AI Intraday Picks with criteria tracking."""
    if df.empty or len(df) < 20:
        return None, {}
    try:
        close = df['Close'].astype(float)
        ...
        criteria = {
            'Volume Surge >1.2x': vol_condition,
            'Price > 20 MA': price_ma_condition,
            'RSI between 30-70': rsi_condition,
            'AI Confidence > 60%': ai_confidence > 0.6,
            'Combined Score ≥ 3': combined_score >= 3
        }
...
def intraday_picks():
    ...
    for name, ticker in ALL_STOCKS.items():
        ...
        sig, _ = ai_intraday_signal(df, name)
        if sig:
            picks.append(sig)
...
def main_app():
    ...
    with screener_tab1:
        ...
        st.caption("AI-powered swing signals combining technical rules with RandomForest. Click on any stock to see criteria details.")
        ...
        if swing_data:
            ...
            st.dataframe(swing_df.style.apply(highlight_rows, axis=1), width='stretch')
            
            if swing_data:
                selected_stock = st.selectbox("View criteria for stock:", swing_df['Stock'].tolist(), key="swing_criteria")
                if selected_stock in swing_criteria:
                    st.markdown(display_criteria_table(swing_criteria[selected_stock], f"Criteria Status for {selected_stock}"), unsafe_allow_html=True)
        else:
            no_stocks_message(
                "AI Swing Scanner",
    ...
    with screener_tab2:
        ...
        st.caption("High probability pullback opportunities. Click on any stock to see criteria details.")
        ...
        if pullback_data:
            ...
            st.dataframe(pullback_df.style.apply(highlight_top, axis=1), width='stretch')
            
            selected_stock = st.selectbox("View criteria for stock:", pullback_df['Stock'].tolist(), key="pullback_criteria")
            if selected_stock in pullback_criteria:
                st.markdown(display_criteria_table(pullback_criteria[selected_stock], f"Criteria Status for {selected_stock}"), unsafe_allow_html=True)
        else:
            no_stocks_message(
    ...
    with screener_tab3:
        ...
        st.caption("Breakout opportunities. Click on any stock to see criteria details.")
        ...
        if breakout_data:
            ...
            st.dataframe(breakout_df.style.apply(highlight_top, axis=1), width='stretch')
            
            selected_stock = st.selectbox("View criteria for stock:", breakout_df['Stock'].tolist(), key="breakout_criteria")
            if selected_stock in breakout_criteria:
                st.markdown(display_criteria_table(breakout_criteria[selected_stock], f"Criteria Status for {selected_stock}"), unsafe_allow_html=True)
        else:
            no_stocks_message(
    ...
    with screener_tab4:
        ...
        st.caption("Real-time 5-minute breakout signals. Click on any stock to see criteria details.")
        ...
        if intraday_breakout_data:
            ...
            st.dataframe(intraday_breakout_df.style.apply(highlight_top, axis=1), width='stretch')
            
            selected_stock = st.selectbox("View criteria for stock:", intraday_breakout_df['Stock'].tolist(), key="intraday_breakout_criteria")
            if selected_stock in intraday_breakout_criteria:
                st.markdown(display_criteria_table(intraday_breakout_criteria[selected_stock], f"Criteria Status for {selected_stock}"), unsafe_allow_html=True)
        else:
            no_stocks_message(
    ...
    with screener_tab5:
        ...
        st.caption("AI‑powered intraday picks. Higher score = stronger signal. Click on any stock to see criteria details.")
        ...
        if intraday:
            ...
            st.dataframe(intraday_df.style.apply(highlight_top, axis=1), width='stretch')
            
            selected_stock = st.selectbox("View criteria for stock:", intraday_df['Stock'].tolist(), key="ai_intraday_criteria")
            if selected_stock in intraday_criteria:
                st.markdown(display_criteria_table(intraday_criteria[selected_stock], f"Criteria Status for {selected_stock}"), unsafe_allow_html=True)
        else:
            no_stocks_message(
                "AI Intraday Picks",
    ...
    if st.session_state.holdings_df is not None and not st.session_state.holdings_df.empty:
        ...
        with col1:
            ...
            <div class="metric-card">
                ...
                <div class="metric-value">{total_val_display}</div>
            </div>
            """, unsafe_allow_html=True)
        ...
        with col2:
            if st.session_state.total_cost > 0:
                ...
                <div class="metric-card">
                    ...
                    <div class="metric-value">{pnl_display}</div>
                    <div class="metric-delta" style="color:{delta_color};">{pnl_pct_display}</div>
                </div>
                """, unsafe_allow_html=True)
            else:
                st.markdown(f"""
